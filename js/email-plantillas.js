...
<div style="font-family: Arial, sans-serif; background-color: #f4f7fa; padding: 20px;">
  ...
  // Genera { subject, html, text } para el siniestro dado (ROTURA o FALTA),
  ...
  function plantillaSiniestro(s, fechaInformeISO) {
    ...
    const esRotura = s.tipo === 'ROTURA';

    const html = esRotura
      ? plantillaHtmlRotura(tienda, fechaInformeStr, fechaLimite)
      : plantillaHtmlFalta(tienda, fechaInformeStr);

    const subject = esRotura
      ? `Aviso de roturas en la entrega de mercancía – ${tienda.toUpperCase()} – ${fechaInformeStr}`
      : `Aviso de faltas en la entrega de mercancía – ${tienda.toUpperCase()} – ${fechaInformeStr}`;

    const text = esRotura
      ? `Se han detectado roturas en la mercancía recibida en la tienda ${tienda} (recepción ${fechaInformeStr}). Fecha límite de recogida para reclamación al seguro: ${fechaLimite}.`
      : `Se han detectado faltas en la mercancía recibida en la tienda ${tienda} (recepción ${fechaInformeStr}).`;
